...
# class VinAILandmarkerDataSet(data.Dataset):
...
#         # It will be an dict look like {"png": [68 lmks]  }
#         self.label =  self.__loadlabel__(anno_dir)
#         self.img_path_list = glob.glob(f'{img_dir}/*.png')
     
#         self.TARGET_IMAGE_SIZE = (imgsize, imgsize)


#     def __loadlabel__(self, anno_dir):
#         label = {}
...
#                 line_pt = line.split(" ")
#                 base_file_name = line_pt[0]
#                 lmks = [float(i) for i in line_pt[3:]]
#                 lmks = np.array(lmks).reshape(-1,2)
#                 label[base_file_name] = lmks
...
#         self.landmark = self.label[os.path.basename(f)]
...
class VinAILandmarkerDataSet(data.Dataset):
    ...
    def __init__(self, img_dir, anno_file, augment=False, transforms=None, imgsize=256, set_type="train"):
        self.img_dir = img_dir
        ...
        self.pngs, self.lmks =  self.__loadlabel__(anno_file)

        if set_type == 'train':
            self.pngs = self.pngs[:int(0.95*len(self.pngs))]
            self.lmks = self.lmks[:int(0.95*len(self.lmks))]
        ...
        self.samling_lmks = self.lmks.copy()
    ...
    def OnSampling(self, num_sample=20000):
        ...
        self.samling_lmks = np.array(self.lmks)[indices]
    ...
    def __loadlabel__(self, anno_file):
        pngs = []
        lmks_list = []
        with open(anno_file, 'r') as f:
            while f:
                ...
                png = os.path.dirname(self.img_dir) + "/" + base_file_name

                if len(lmks) != 68:
                    print(f'Lmks of this {anno_file} is not 68')
                    continue
    ...
    def  __getitem__(self, index):
        ...
        self.img = cv2.imread(f)
        self.landmark = self.samling_lmks[index]
...
if __name__ == "__main__":
    ...
    # dataset = VinAILandmarkerDataSet(img_dir="/home/vuthede/Downloads/task_pilot - test (5)-2021_07_08_08_58_30-cvat for images 1.1/images",
    #             anno_dir="/home/vuthede/Downloads/list_ExtractData_label.txt",
    #             augment=False,
    #             transforms=None, set_type="train")

    lapa = VinAILandmarkerDataSet(img_dir="/home/ubuntu/vuthede/delivery_17082021",
            anno_file="/home/ubuntu/vuthede/delivery_17082021/labels/landmark_annotations.txt",
            augment=False,
    ...
    debug_vinai_lmks = "debug_vinai_lmks"
