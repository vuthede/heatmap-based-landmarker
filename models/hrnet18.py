...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, num_modules, num_branches, block, 
        ...
        self.con_conv1 =  conv_bn(270, 140, 3 , 2, 1)
        
        self.con_conv2 =  conv_bn(140, 70, 3 , 2, 1)
         
        self.con_conv3 = conv_bn(70, 70, 3 , 2, 1)
        
        self.con_conv4 = conv_bn(70, 35, 3 , 2, 1)
    ...
    def forward(self, x):
        ...
        concat_fea = torch.cat([F.interpolate(y,**kwargs) for y in y_list], 1)

        # Constraint face shape
        fea = self.con_conv1(concat_fea)
        fea = self.con_conv2(fea)
        fea = self.con_conv3(fea)
        fea = self.con_conv4(fea)
        fea = fea.view(fea.size(0), -1)
        lmks_regress = self.lmks_regress(fea)

        return concat_fea, lmks_regress
